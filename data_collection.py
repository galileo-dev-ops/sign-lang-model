...
with mp.solutions.holistic.Holistic(min_detection_confidence=0.75, min_tracking_confidence=0.75) as holistic:
    ...
    for action, sequence, frame in product(actions, range(sequences), range(frames)):
        # If it is the first frame of a sequence, wait for the space-bar key press to start recording
        if frame == 0:
            while True:
                ...
                results = image_process(image_copy, holistic)
                if results.left_hand_landmarks is not None:
                    mp.solutions.drawing_utils.draw_landmarks(image_copy, results.left_hand_landmarks,
                                                              mp.solutions.holistic.HAND_CONNECTIONS)
                if results.right_hand_landmarks is not None:
                    mp.solutions.drawing_utils.draw_landmarks(image_copy, results.right_hand_landmarks,
                                                              mp.solutions.holistic.HAND_CONNECTIONS)

                cv2.putText(image_copy, 'Recording data for the "{}". Sequence number {}.'.format(action, sequence),
                ...
                cv2.putText(image_copy, 'Press "Space" when you are ready.', (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 1,
                   (0, 0, 255), 2, cv2.LINE_AA)
                cv2.imshow('Camera', image_copy)

                if cv2.waitKey(1) == 32:  # ASCII value for space-bar is 32
                    break
